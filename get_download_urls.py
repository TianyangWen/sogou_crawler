...
def get_main_categories():
    ...
    links = page_soup.find_all('div',attrs={"class":"dict_category_list_title"})
    main_categories=[]
    for link in links:
...
def collect_urls(single_category):
    download_links=[]
    for i in range(2000):
        new_links=detect_download_links(single_category+'/default/'+str(i+1))
        if new_links:
            download_links+=new_links
        else:
            break
    with open('./dict_download_urls/'+single_category[41:]+'.txt','w',encoding='utf-8')as f:
        f.writelines(download_links)
    print('INFO:Collected '+str(len(download_links))+' urls')
    
categories=get_main_categories()
extend_categories=['https://pinyin.sogou.com/dict/cate/index/180',#北京
